...
function GMLH(width, height) {
    ...
    document.addEventListener('mouseup', e => e.preventDefault(), false);
    ...
    PIXI.Loader.shared.add([
        ...
        let gridScale = width / 50;
        for (let i = 0; i < gridScale; i++) {
            let lw = 2;
            if (!(i % 100)) {lw = 5;}
            else if (!(i % 25)) {lw = 4;}
            else if (!(i % 5)) {lw = 3;}
            gridLines.lineStyle(lw, 0x000000, 0.75);
            gridLines.moveTo(-width / 2, i * 50 - height / 2);
            gridLines.lineTo(width / 2, i * 50 - height / 2);
            gridLines.moveTo(i * 50 - width / 2, -height / 2);
            gridLines.lineTo(i * 50 - width / 2, height / 2);
            gridLines.endFill();
        }
    ...
    const keys = {
        ...
        'w': keyboard('w', event => {
            zoom(held || selected || null, 1 / 0.95);
        }),
        's': keyboard('s', event => {
            zoom(held || selected || null, 0.95);
        }),
        'r': keyboard('r', event => {
